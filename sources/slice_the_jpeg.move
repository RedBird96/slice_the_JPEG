...
module overmind::slice_the_jpeg {
    ...
    fun init_module(account: &signer) {
        ...
        let collection_address = create_split_collection(&account_resource_signer);

        // TODO: Create a new State object and send it to account

        let split_events = account::new_event_handle<SplitEvent>(&account_resource_signer);
        let redeem_events = account::new_event_handle<RedeemEvent>(&account_resource_signer);
        let call_events = account::new_event_handle<CallEvent>(&account_resource_signer);
        let exchange_events = account::new_event_handle<ExchangeEvent>(&account_resource_signer);
    ...
    public entry fun split<NFT: key >(
        ...
        check_if_account_owns_nft(nf_token_owner_address, );
        ...
        let state = borrow_global<State>(nf_token_owner_signer);
        ...
        check_if_split_token_does_not_exist(token_address);
        ...
        let token = object::address_to_object(token_address);
        object::transfer(nf_token_owner_signer, token, resource_account_address);
    ...
    inline fun check_if_account_owns_nft<NFT: key>(account_address: address, nf_token_object: Object<NFT>) {
       assert!(account_address != nf_token_object.owner, EAccountDoesNotOwnNft);
    }
    ...
    inline fun check_if_split_token_does_not_exist(split_token_address: address) {
        assert!(exist(split_token_address), ESplitTokenAlreadyExists);
    }
